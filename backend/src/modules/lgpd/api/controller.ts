...
export class LGPDController {
  ...
  async listConsentimentos(req: Request, res: Response) {
    try {
      ...
      const data = await (prisma as any).lgpd_consentimentos.findMany({
        where,
  ...
  async createConsentimento(req: Request, res: Response) {
    try {
      const data = await (prisma as any).lgpd_consentimentos.create({
        data: req.body,
      });
  ...
  async listSolicitacoes(req: Request, res: Response) {
    try {
      ...
      const data = await (prisma as any).lgpd_solicitacoes.findMany({
        where,
        orderBy: { created_at: "desc" },
      });
      res.json(data);
  ...
  async createSolicitacao(req: Request, res: Response) {
    try {
      const data = await (prisma as any).lgpd_solicitacoes.create({
        data: req.body,
      });
      res.status(201).json(data);
    } catch (error: any) {
  ...
  async updateSolicitacao(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const data = await (prisma as any).lgpd_solicitacoes.update({
        where: { id },
        data: req.body,
      });
      res.json(data);
