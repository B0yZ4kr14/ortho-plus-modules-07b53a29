...
export function useAgendaApi() {
  ...
  useEffect(() => {
    ...
    if (user && selectedClinic) {
      ...
      // Replace realtime subscription with basic polling (every 30s)
  ...
  const addAppointment = async (
    ...
    try {
      ...
      const startDateTime = new Date(
        `${appointment.data}T${appointment.horaInicio}`,
      );
      const endDateTime = new Date(
        `${appointment.data}T${appointment.horaFim}`,
      );
