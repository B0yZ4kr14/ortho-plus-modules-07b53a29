...
export default function ContasReceber() {
  const { contasReceber, loading, addContaReceber } = useFinanceiro();
  ...
  const filteredContas = useMemo(() => {
    return contasReceber.filter((conta) => {
      ...
      const matchesStatus =
        filterStatus === "todas" || conta.status === filterStatus;
  ...
  const handleEnviarCobranca = async (
    ...
    try {
      ...
      await apiClient.post("/api/faturamento/enviar-cobranca", {
        cobrancaId: contaId,
        tipo,
      });
  ...
  const exportarPDF = () => {
    ...
    doc.text(
      `Total a Receber: ${new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(totalReceber)}`,
      14,
      40,
    );
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <div className="flex gap-2">
              <Select
                onValueChange={(value) =>
                  value === "pdf" ? exportarPDF() : exportarExcel()
                }
              >
