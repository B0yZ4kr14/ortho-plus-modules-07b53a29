    ...
    <div className="space-y-6">
          ...
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-muted-foreground mt-2">
                Configure webhooks no repositório GitHub para receber
                      ...
                      {event.event_data && (
                        <div className="mt-2">
                          {event.event_data.commits &&
                            Array.isArray(event.event_data.commits) && (
                              <p className="text-sm">
                                <span className="font-medium">
                                  {event.event_data.commits.length}
                                </span>{" "}
                                commit(s) recebido(s)
                              </p>
                            )}
                          {event.event_data.pull_requests && (
                            <p className="text-sm">
                              PR #{event.event_data.pull_requests[0]?.number}:{" "}
                              {event.event_data.pull_requests[0]?.title}
                            </p>
                          )}
                          {event.event_data.workflows && (
                            <p className="text-sm">
                              Workflow: {event.event_data.workflows[0]?.name} -
                              Status: {event.event_data.workflows[0]?.status}
                            </p>
                          )}
                        </div>
                      )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure webhooks no seu repositório GitHub para receber
