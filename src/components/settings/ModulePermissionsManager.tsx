...
export function ModulePermissionsManager() {
  ...
  const fetchData = async () => {
    try {
      ...
      const modulesData = await apiClient.post<any>(
  ...
  const hasPermission = (userId: string, moduleId: number) => {
    return permissions.some(
      (p) =>
        p.user_id === userId && p.module_catalog_id === moduleId && p.can_view,
    );
  };
  ...
  const togglePermission = async (userId: string, moduleId: number) => {
    ...
    try {
      ...
      if (currentPermission) {
        ...
        setPermissions([
          ...permissions,
          { user_id: userId, module_catalog_id: moduleId, can_view: true },
        ]);
    ...
    return (
      <Card className="p-8 text-center">
        ...
        <h3 className="text-lg font-semibold mb-2">
          Nenhum usuário MEMBER encontrado
        </h3>
        <p className="text-sm text-muted-foreground">
          Cadastre usuários com role MEMBER para gerenciar suas permissões de
          acesso aos módulos.
        </p>
      </Card>
    );
                    ...
                    {categoryModules.map((module) => (
                      ...
                      >
                        <Checkbox
                          ...
                          onCheckedChange={() =>
                            togglePermission(selectedUser, module.id)
                          }
                          id={`module-${module.id}`}
                        />
                        <label
